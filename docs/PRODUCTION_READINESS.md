...
2. **Backends de staging/dev hardcoded (sem camada de ambiente).** — 🟡 **PARCIALMENTE RESOLVIDO**
   - ✅ **Camada de ambiente criada:** `src/config/env.ts` resolve as 3 URLs base por ambiente (`travelCashApi`, `tripEdgeSearch`, `tripEdgeSite`), selecionado por `EXPO_PUBLIC_APP_ENV`. Os 13 serviços agora importam de `endpoints` — sem URL literal solta. `eas.json`: `development`/`preview` → staging; `production` → production.
   - ✅ **TravelCash API prod definido:** `travelcash-api-prd.azurewebsites.net` (o de staging permanece `-stg`).
   - ⏳ **Pendente:** URLs de **produção da TripEdge** (`tripEdgeSearch` e `tripEdgeSite`) ainda não definidas — o bloco `production` as deixa vazias de propósito (falham alto em vez de apontar pra `-dev`). Preencher em `src/config/env.ts` antes de um build de produção.
   - ⏳ **Pendente (secret):** em produção a TripEdge ainda usa `TRIPEDGE_PARTNER_KEY` SANDBOX e `PARTNER_SSO_SECRET` placeholder — resolver junto do backend proxy (item 1 / Fase 2).
